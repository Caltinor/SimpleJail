...
public class EventHandler {
	...
	public static void onCommand(CommandEvent event) throws CommandSyntaxException {
		if (event.getParseResults().getContext().getSource().hasPermissionLevel(2)) return;
		UUID pid = null;
		try {pid = event.getParseResults().getContext().getSource().asPlayer().getUniqueID();}
		catch (CommandSyntaxException e) {return;}
		if (WSD.get(event.getParseResults().getContext().getSource().getWorld()).getJailed().containsKey(pid) 
	...
	public static void onServerTick(ServerTickEvent event) {		
		if (event.phase.equals(Phase.START) /*&& event.side.equals(LogicalSide.SERVER)*/) {
			for (ServerWorld world : SimpleJail.jailServer.getWorlds())
					...
					for (Map.Entry<UUID, Sentence> population : WSD.get(world).getJailed().entrySet()) {
						...
						//check for out of place players and return them to jail
							...
							if (!player.getServerWorld().equals(world)) {
								player.teleport(world, p.getX(), p.getY(), p.getZ(), player.cameraYaw, player.rotationPitch);
							}
							else if (c.distanceSq(p.getX(), p.getY(), p.getZ(), true) >= leash) {
